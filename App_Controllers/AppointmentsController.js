...
(function () {
    ...
    function AppointmentsController($scope, $http, $location, $window, $timeout) {
        ...
        $scope.pageCount = function () {
            if ($scope.custname) {
                var filtereddata = $filter('filter')($scope.appointdetails, $scope.custname);
            ...
            if ($scope.petname) {
                var filtereddata1 = $filter('filter')($scope.appointdetails, $scope.petname);
            ...
            if ($scope.service) {
                var filtereddata2 = $filter('filter')($scope.appointdetails, $scope.service);
                return Math.ceil(filtereddata2.length / $scope.itemsPerPage) - 1;
            }
            else if (!$scope.service) {
                return Math.ceil($scope.appointdetails.length / $scope.itemsPerPage) - 1;
            }
            if ($scope.servicetype) {
                var filtereddata3 = $filter('filter')($scope.appointdetails, $scope.servicetype);
                return Math.ceil(filtereddata3.length / $scope.itemsPerPage) - 1;
            }
        ...
        $scope.appoinfltr = function (aptname, idno, custmrid, apids,invoiceNo) {
           ...
           $scope.Confrmid1 = { "appointment_name": aptname, "appointment_id": idno };
            ...
            $http({
                ...
                url: 'http://166.62.84.37/PetzCareWS/ServiceInvoice.svc/GetServiceInvoiceReport',
                headers: {
                    "Content-Type": 'application/json',
                }
            }).success(function (response) {
