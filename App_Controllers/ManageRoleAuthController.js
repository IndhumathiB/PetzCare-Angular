(function () {
    ...
    function ManageRoleAuthController($scope, $http, $location, $rootScope, $filter, $timeout) {
        ...
        $scope.add = function () {
            ...
            if (!$scope.roleForm.$invalid) {
                $http({
                    method: 'POST',
                    data: $scope.data1,
                    url: 'http://166.62.84.37/PetzCareWS/AppSecurity.svc/AddRoleAuthorization',
                    headers: {
        ...
        //show role
            ...
            $http({
                method: 'GET',
                url: 'http://166.62.84.37/PetzCareWS/AppSecurity.svc/GetAllRoleAuthorizationMappings',
                headers: {
                    "Content-Type": 'application/json',
                }
            }).success(function (response) {
        ...
        $scope.search = function () {
            $scope.filteredItems = $filter('filter')($scope.items, function (item) {
                for (var attr in item) {
                    if (searchMatch(item[attr], $scope.searchingvalue))
                        return true;
                }
                return false;
            });
        ...
        $scope.range = function (start,end) {
            //console.log($scope.groupToPages());
            //var rangeSize = $scope.filteredItems.length;
            //    if (rangeSize >= 6)
            //    {
            //        rangeSize = 5;
            //    }
            ...
            //    if (start > $scope.pagedItems - rangeSize) {
            //        start = $scope.pagedItems - rangeSize + 1;
            //    }
        ...
        $scope.DisableNextPage = function () {
            return $scope.currentPage === end ? "disabled" : "";
            };
        ...
        $scope.edit = function (id_no, selects) {
            ...
            if (editRole == 1) {
                ...
                var getdata = $http({
                    method: 'POST',
                    data: $scope.Roleid,
                    url: 'http://166.62.84.37/PetzCareWS/AppSecurity.svc/GetRoleAuthorizationMappingById',
                    headers: {
                        "Content-Type": 'application/json',
                    }
        ...
        var array = [];
            ...
            if (!$scope.editForm.$invalid) {
                $http({
                    method: 'POST',
                    data: $scope.data2,
                    url: 'http://166.62.84.37/PetzCareWS/AppSecurity.svc/UpdateRoleAuthorization',
                    headers: {
                        "Content-Type": 'application/json',
                    }
                }).success(function (response) {
